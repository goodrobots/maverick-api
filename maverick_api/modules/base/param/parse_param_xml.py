...
def download_param_meta(
    url, timeout=(3.0, 5.0)
):  # 3 second connect and 5 second read timeout
    ...
    try:
        ...
        print(
            "Could not retreive param meta data from remote server: {0} {1}".format(
                url, e
            )
        )
    finally:
...
def extract_text(node):
    ...
    return re.sub(
        "\s+", " ", node.text
    )  # replace newlines, tabs and white space with single space
...
def run_bool_test(curr_param):
    ...
    if (
        ...
        if len(curr_param["values"]) == 2 and curr_param["values"].keys() == [
            0,
            1,
        ]:  # Can only contain two keys and they must be 0 and 1
            if (
                "DISA" in curr_param["values"][0].upper()
                and "ENAB" in curr_param["values"][1].upper()
            ) or (
                "NORM" in curr_param["values"][0].upper()
                and "REVE" in curr_param["values"][1].upper()
            ):
                curr_param["values"] = {
                    bool_true: curr_param["values"][1],
                    bool_false: curr_param["values"][0],
                }
...
def extract_param_meta_from_tree_px4(tree, vehicle):
    ...
    for group in groups:
        for sub in group.iter():
            ...
            if sub.tag == "group":
                curr_group_name = sub.attrib[
                    "name"
                ]  # we enforce that there is a group name
            elif sub.tag == "parameter":
                if (
                    curr_param_dict
                ):  # we have already populated curr_param_dict, write it to the root and reset
                ...
                curr_param_dict["group"] = (
                    curr_param_name.split("_")[0].strip().rstrip("_").upper()
                )
                ...
                curr_param_dict["values"][
                    ast.literal_eval(sub.attrib["code"].encode("utf-8").strip())
                ] = sub.text.encode("utf-8")
            elif sub.tag == "bit":
                if curr_param_dict["bitmask"] is None:
                    curr_param_dict["bitmask"] = {}
                tag_string = extract_text(sub)
                curr_param_dict["bitmask"][
                    2 ** int(sub.attrib.get("index"))
                ] = tag_string
    ...
    for node in nodes:
        for sub in node.iter():
            if sub.tag == "parameters":
                ...
                curr_param_dict = {
                    "humanName": sub.attrib.get("humanName", None),
                    "humanGroup": None,
                    "rebootRequired": False,
                    "increment": None,
                    "unitText": None,
                    ...
                    "max": None,
                    "values": None,
                    "documentation": sub.attrib.get("documentation", None),
                    "bitmask": None,
                    "decimal": None,
                    "type": None,
                }
            ...
            # process all 'fields', range, increment, units, unitText, etc...
                ...
                if name == "RANGE":
                    ...
                    if len(param_range) != 2:
                        ...
                        curr_param_dict["min"] = ast.literal_eval(
                            param_range[0].strip()
                        )
                        curr_param_dict["max"] = ast.literal_eval(
                            param_range[1].strip().lstrip("+")
                        )
                elif name == "UNITS":
