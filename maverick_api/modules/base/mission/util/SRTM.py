...
class SRTM(object):
    ...
    def set_srtm_cache_dir(self, srtm_cache_dir):
        if not os.path.exists(srtm_cache_dir):
            self.logger.info("SRTM cache path doesn't exist = {0}".format(srtm_cache_dir))
        else:
...
class NEDGround(object):
    def __init__(self, lla_ref, width_m, height_m, step_m, dict_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'srtm'), logger = None):
        if logger is None:
            self.logger = common.stdoutLogger()
        else:
            self.logger = logger
