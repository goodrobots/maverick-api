...
def lla_ll_corner(lat_deg, lon_deg):
    return int(np.floor(lat_deg)), int(np.floor(lon_deg))
...
class SRTM(object):
    def __init__(self, lat, lon, dict_path, logger=None):
        if logger is None:
            self.logger = common.stdoutLogger()
        else:
            self.logger = logger
    ...
    def load_srtm_dict(self, dict_path):
        ...
        try:
            ...
            self.logger.error("Unable to read = {0}".format(dict_file))
    ...
    def set_srtm_cache_dir(self, srtm_cache_dir):
        if not os.path.exists(srtm_cache_dir):
            self.logger.info(
                "SRTM cache path doesn't exist = {0}".format(srtm_cache_dir)
            )
        else:
    ...
    def download_srtm(self, fileroot):
        ...
        if fileroot in self.srtm_dict:
            ...
            self.logger.info(file.retrieve(url, download_file))
            return True
        else:
            self.logger.info("Requested srtm that is outside catalog")
            return False
    ...
    def parse(self):
        ...
        if not os.path.exists(cache_file):
            if not self.download_srtm(tilename):
                return False
        self.logger.info("Parsing SRTM file: {0}".format(cache_file))
    ...
    def make_lla_interpolator(self):
        self.logger.info("Constructing LLA interpolator")
        ...
        # print x
        # print y
        self.lla_interp = scipy.interpolate.RegularGridInterpolator(
            (x, y), srtm_pts, bounds_error=False, fill_value=-32768
        )
...
class NEDGround(object):
    def __init__(
        ...
        height_m,
        step_m,
        dict_path=os.path.join(os.path.dirname(os.path.realpath(__file__)), "srtm"),
        logger=None,
    ):
        if logger is None:
            self.logger = common.stdoutLogger()
    ...
    def load_tiles(self, lla_ref, width_m, height_m):
        self.logger.info("Loading DEM tiles")
        ...
        for lat in range(lat1, lat2 + 1):
            for lon in range(lon1, lon2 + 1):
                srtm = SRTM(lat, lon, self.dict_path, logger=self.logger)
                if srtm.parse():
    ...
    def make_interpolator(self, lla_ref, width_m, height_m, step_m, load=False):
        self.logger.info("Constructing NED area interpolator")
        if load:
            pass  # load n_list, e_list, ned_ds here
        ...
        do_save = False
        if do_save:
            pass  # save n_list, e_list, ned_ds here
