...
class MaverickSchema(schemaBase):
    def __init__(self):
        ...
        self.configure_command_defaults = {
            "enviroment":None,
            "module":None, "debug":False, "profile":False, "dryrun":False,
            "running":False, "uptime":None,
            "stdout":None, "stderror":None,
            "returncode":None,
        }
        ...
        self.configure_command_type = GraphQLObjectType(
            ...
            lambda: {
                "enviroment": GraphQLField(
                    GraphQLString, description="Environment to configure: bootstrap, dev, flight or minimal"
                ),
                "dryrun": GraphQLField(GraphQLBoolean, description="Configure dry run - just show what would be done"),
                "debug": GraphQLField(GraphQLBoolean, description="Debug output"),
                "profile": GraphQLField(GraphQLBoolean, description="Profile performance data"),
                "module": GraphQLField(GraphQLString, description="Only make changes to module <module>"),
                "running": GraphQLField(GraphQLBoolean, description=""),
                "uptime": GraphQLField(GraphQLInt, description="Number of seconds the process has been running for"),
    ...
    def sub_configure_command_status(self, root, info):
        return EventEmitterAsyncIterator(
            self.subscriptions, "modules.api.maverick.MaverickSchema" + "MaverickConfigure"
        )
    ...
    async def run(self, cmd):
        ...
        application_log.debug(f'[{cmd!r} exited with {self.configure_command["returncode"]}]')
