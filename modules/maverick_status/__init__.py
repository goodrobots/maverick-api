...
def get_script():
    status_messages = []
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "script.txt"), "r+") as fid:
        for line in fid:
            if not line.isspace():
...
status_data = dict(id=uuid4().__str__(), currentStatus="...", currentTime=str(monotonic_time()), status_messages=get_script(), status_count = 0)
...
class StatusSchema(schemaBase):
    def __init__(self):
        ...
        self.report_type = GraphQLObjectType(
            ...
            lambda: {
                "id": GraphQLField(
                    GraphQLNonNull(GraphQLString), description="The id of the API instance."
                ),
                "currentStatus": GraphQLField(GraphQLString, description="The API status."),
        ...
        self.q = {
            "Status": GraphQLField(
                self.report_type,
                resolve=self.get_report,
            )
        }
        
        self.s = {"Status": GraphQLField(self.report_type, subscribe=self.sub_report, resolve=None)}
    
    def get_report(self, root, info):
        """API status report query handler"""
        status_data["currentTime"] = str(monotonic_time())
        (status_data["currentStatus"], status_data["status_count"]) = get_status(status_data["status_messages"], status_data["status_count"])
...
class StatusModule(moduleBase):
    ...
    def install_periodic_callbacks(self):
        ...
        self.periodic_callbacks.append(tornado.ioloop.PeriodicCallback(callback, callback_time=1000, jitter=0.1))
