...
class Session(object):
    ...
    async def is_authenticated(self, client):
        ...
        document = await db.session_collection.find_one({"session_id":self.session_id.decode('utf-8')})
            ...
            async def wrapper_authenticated(self, *args, **kwargs):
                ...
                if RBAC is not None:
                    ...
                    # The RBAC requirements have been met
                        ...
                        return GraphQLError(message="Current user is not authorized")
                else:
                    return await func(self, *args, **kwargs)
        ...
        async def wrapper_active_session(self, *args, **kwargs):
            ...
            if not session_id:
                ...
                try:
                    ...
                    result = await db.session_collection.insert_one(self.current_user.create_document())
